...
uint8_t flag_FlashUpateFinish = 0;
uint8_t flag_YmodemDataReceive = 0;
uint8_t flag_YmodemDataTimeOut = 0;
/* Private function prototypes -----------------------------------------------*/
void SerialDownload(void);
void SerialUpload(void);

/* Private functions ---------------------------------------------------------*/
void DelayYmodem(__IO uint32_t nCount)
{
...
//  SerialPutString("Waiting for the file to be sent ... (press 'a' to abort)\n\r");
  ...
  if (Size > 0)
  {
    ...
    SerialPutString("-------------------\n");
	#else
    SerialPutString("\n\n\rB)Programming Completed Successfully\n\r");
	DelayYmodem(100);
	#endif

	flag_YmodemDataReceive = 1;
	flag_FlashUpateFinish = 1;
  }
  else if (Size == -1)
  {
    SerialPutString("\n\n\rB)The image size is higher than the allowed space memory!\n\r");
  }
  else if (Size == -2)
  {
...
void SerialUpload(void)
{
  ...
  if (GetKey() == CRC16)
  {
    ...
    if (status != 0) 
    {
      SerialPutString("\n\rB)Error Occurred while Transmitting File\n\r");
    }
    else
    {
      SerialPutString("\n\rB)File uploaded successfully \n\r");
    }
  }
}
